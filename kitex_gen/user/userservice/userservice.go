...
type UserInfoResult struct {
	Success *user.UserInfoRes
}

var UserInfoResult_Success_DEFAULT *user.UserInfoRes

func (p *UserInfoResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(user.UserInfoRes)
	}
	return p.Success.FastRead(buf, _type, number)
}
...
func (p *UserInfoResult) GetSuccess() *user.UserInfoRes {
	if !p.IsSetSuccess() {
